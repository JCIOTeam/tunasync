...
/// Full version string, set at compile time by the build script.
/// Format: `PKG_VERSION, build YYYY-MM-DD, git SHORT_SHA`
/// (git SHA omitted when not built from a git clone).
pub const VERSION: &str = env!("TUNASYNC_VERSION");
